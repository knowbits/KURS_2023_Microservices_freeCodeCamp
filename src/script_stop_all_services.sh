...
# Stop each service using "kubectl delete ./":
echo;
cd "$DIR_MYSQL"; kubectl delete -f ./;
cd "$DIR_RABBITMQ"; kubectl delete -f ./;
cd "$DIR_MONGODB"; kubectl delete -f ./;
cd "$DIR_AUTH"; kubectl delete -f ./;
cd "$DIR_GATEWAY"; kubectl delete -f ./;
cd "$DIR_CONVERTER"; kubectl delete -f ./;
cd "$DIR_NOTIFICATION"; kubectl delete -f ./;

# Print the status of our minikube pods

KUBECTL_GET_PODS_OUTPUT=$(kubectl get pods -o wide);
echo;
echo "Status of our Kubernetes pods after STOPPING services:"
echo;
